...
task process_vcf {
    ...
    runtime {
        docker: "gcr.io/nitrogenase-docker/phenics:0.2.51"
        memory: "16 GB"
    ...
    command <<<
        ...
        phenics gcs-sample -d ~{vcf_file} -i ~{index} -p ~{phenotypes_file} -r ~{interval_size} -x ~{max_step_size} \
                           -o ~{output_file_name}
    >>>
    output {
...
task merge {
    ...
    command <<<
        set -e
        phenics merge -i ~{sep=' ' liabilities_files} -o merged
